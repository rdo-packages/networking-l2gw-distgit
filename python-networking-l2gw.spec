...
Requires:   python-%{pypi_name} = %{version}-%{release}
Summary:    Neutron L2GW Agent
%description -n %{servicename}-agent
Agent that enables L2GW functionality
...
%post openstack-%{servicename}-agent
%systemd_post %{servicename}-agent.service

%preun openstack-%{servicename}-agent
%systemd_preun %{servicename}-agent.service

%postun openstack-%{servicename}-agent
...
%exclude %{_unitdir}/%{servicename}-agent.service
%exclude %{_bindir}/neutron-l2gateway-agent
