...
mv %{buildroot}/usr/etc/neutron/*.ini %{buildroot}%{_sysconfdir}/neutron/conf.d/

# Make sure neutron-server loads new configuration file
mkdir -p %{buildroot}/%{_datadir}/neutron/server
ln -s %{_sysconfdir}/neutron/conf.d/l2gw_plugin.ini %{buildroot}%{_datadir}/neutron/server/l2gw_plugin.ini
...
%config(noreplace) %attr(0640, root, neutron) %{_sysconfdir}/neutron/conf.d/l2gateway_agent.ini
%config(noreplace) %attr(0640, root, neutron) %{_sysconfdir}/neutron/conf.d/l2gw_plugin.ini
